...
# filled per each metric  
merged_all_dataExp1 <- NULL
merged_all_data <- NULL  
...
# Set the main mapper to Exp4.
expMode <- "Exp4"
# Needed for IVDP calculated that get joined latter with CVDP. 
enableIVDP_one_release <- TRUE

###########################Start Loading Results Loop ############################
#################################################################################

#To reproduce the TSE results you show follow the configs belwo:
# expMode <- "Exp4"  // CVDP is main varriable that get joined with IVDP later. 
# enableIVDP_one_release <- TRUE // needed to calculate the performance for IVDP
# expModeOptions<- c("Exp1", "Exp4") // You need to load both Exp1 (IVDP), and Exp4 (CVDP)
...
if (enableIVDP_one_release)
{
  ...
  #stop("Custom message explaining why it stopped")
...
#Must be true to get TSE results
if (enableIVDP_one_release)
{
...
run_sk_esd <- function(data, metric_col) {
  ...
  df_long <- data %>%
     select(DataSet_Tunned, Model_Tunned, metric = .data[[metric_col]])
...
if (enableIVDP_one_release)
{
 ...
 # nrow(merged_all_dataExp1)
 #df_long <- merged_all_dataExp1 %>%
  #select(DataSet_Tunned, Model_Tunned, metric = .data[[IVDPnonTunnedMetricExp1]])
  
 # dup_summary <- df_long %>%
 #   count(DataSet_Tunned, Model_Tunned, name = "n") %>%
  #  filter(n > 1)
  
  # dup_summary
   
  # subset_one <- merged_all_dataExp1 %>%
   #  filter(DataSet_Tunned == "ant-1.4.csv", Model_Tunned == "MLP")
...
# # #########################
 ...
 nrow(sk$m.inf)
  ...
  nrow_train_data <- aggregate(NRowTrainData_Tunned ~ DataSet_NonTunned, 
                               data = ROCsubset_data, FUN = unique)  
  # Assuming there's one unique value per dataset
